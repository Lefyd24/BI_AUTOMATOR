...
def wages(files):
        ...
        for file in files:
            with st.spinner(f"Processing {counter+1} file out of {len(files)}"):
                ...
                try:
                    ...
                    try:
                        df.drop(["Unnamed: 31"], inplace=True)
                    except KeyError as unnamed:
                        print(unnamed)
                        pass
    ...
    #print(dataframes_to_concat)
        ...
        #pd.to_numeric(total_ds["AdjustedGrossWage"])
        total_ds["AdjustedGrossWage"]= total_ds["AdjustedGrossWage"].apply(pd.to_numeric)
        total_ds["DaysOffAgreed"].replace(to_replace="-", value=0, inplace=True)
        print(total_ds)
        ...
        st.warning(f"An error with the following code has occured: {e}.\nMake sure you've uploaded the correct files and if the error persists please contact Lefteris Fthenos to resolve.")
        return 0
